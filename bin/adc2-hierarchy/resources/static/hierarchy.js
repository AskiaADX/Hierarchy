(function () {
	...
	*/
     ...
     * @param {Boolean} [options.useSearch=false] Use the search box, if not then the first level is implicitly visible
	...
	*/
    Hierarchy.prototype.initialize = function initialize() {
        ...
        if (this.useSearch) {
            ...
            this.inputSearch = document.getElementById("hierarchy_search_" + this.instanceId);
            if (!this.inputSearch) {
                return throwError("Could not find the `hierarchy_search_" + this.instanceId + "` input");
            }
	...
	*/
    ...
    Hierarchy.prototype.createOptionsArray = function createOptionsArray(levelIndex) {
        var options = [];
        if (this.dropdowns) {
            options.push('<option value="">' + this.blankOptions[levelIndex] + '</option>');
        }
        return options;
    };
    ...
    Hierarchy.prototype.fillDropdown = function fillDropdown(options) {
        ...
        if (records.length === 1 && this.autoSelect) {
            ...
            for (i = 0, l = records.length; i < l; i++) {
                ...
                opts.push('<option value="' + value + '"'  + attrSelected + '>' + records[i][levelNameIndex] + '</option>');
                added[value] = true;
    ...
    Hierarchy.prototype.clearLevels = function clearLevels(startIndex, duringSearch) {
        var levels = this.levels,
            i = startIndex || 0, l = levels.length;
